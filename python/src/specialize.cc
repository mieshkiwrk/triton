...
// Read a boolean flag off the backend; an absent flag means `absent`.
bool backend_flag(PyObject *backend, PyObject *name, bool absent) {
  auto flag = from_new_ref(PyObject_GetAttr(backend, name));
  if (!flag) {
    PyErr_Clear();
    return absent;
  }
  return PyObject_IsTrue(flag.ptr()) != 0;
}
...
specialize_tensordesc(PyObject *backend, PyObject *arg, bool has_layout,
  ...
  // the flag is an opt-out, so a missing one leaves the native path in place
  if (backend_flag(backend, has_native_tensordesc_spec_attr, true)) {
    return {std::move(type_str_result), py::none()};
  }
...
std::pair<py::object, py::object> handle_tensor(PyObject *backend,
  ...
  bool native_impl_available =
      backend_flag(backend, has_native_tensor_spec_attr, false);
